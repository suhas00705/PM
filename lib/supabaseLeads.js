...
async function getCachedLeads(allowedRegions = []) {
  ...
  while (true) {
    ...
    const res = await fetch(`${LEADS_CACHE_URL}?select=*&order=created_time.desc,id.desc${regionFilter}`, {
...
// Upserts the current list of Sales Engineer profile users from Zoho.
...
// Removes cached engineers that are no longer in the freshly-synced list
// (e.g. someone changed profiles or was deactivated in Zoho).
...
// Reads the cached engineer name list back out (paginated the same way as
// getCachedLeads, using Content-Range rather than assuming a fixed page size).
