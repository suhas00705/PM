...
// Reads cached leads, filtered by region if allowedRegions is non-empty.
//
...
// Using URLSearchParams encodes the full parameter value (including the
// in.() wrapper) as a single opaque string, so PostgREST decodes it correctly.
// Double-quotes around values are also dropped — Supabase JS client doesn't
// use them for text in.() filters and they're not required.
async function getCachedLeads(allowedRegions = []) {
  ...
  while (true) {
    ...
    if (allowedRegions.length > 0) {
      // in.(RegionA,RegionB) — no double-quotes; URLSearchParams handles encoding
      params.append('region', `in.(${allowedRegions.join(',')})`);
    }

    const res = await fetch(`${LEADS_CACHE_URL}?${params.toString()}`, {
      headers: { ...headers, Range: `${from}-${from + REQUEST_SIZE - 1}` }
    });
